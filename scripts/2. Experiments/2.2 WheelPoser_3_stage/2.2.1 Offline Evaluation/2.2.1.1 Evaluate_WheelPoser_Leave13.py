...
WHEELPOSER_IMU2Leaf_config = Config(experiment=experiment_names, model=model_names[0], project_root_dir=".", joints_set=joint_set.WheelPoser, pred_joints_set=joint_set.upper_body,
                normalize=True, r6d=True, loss_type="mse", use_joint_loss=False, mkdir=False, upper_body_only=True, exp_setup='leave_13_out', upsample_copies=7)
...
preds_full_body = preds_tensor
trues_full_body = trues_tensor

preds_full_body[:, joint_set.lower_body] = trues_full_body[:, joint_set.lower_body]
...
preds_aa = rotation_matrix_to_axis_angle(preds_full_body).view(preds_tensor.shape[0], -1)
trues_aa = rotation_matrix_to_axis_angle(trues_full_body).view(trues_tensor.shape[0], -1)
...
with open("./pred_results/end2end_results_combined.pkl", "wb") as f:
    dump = {
