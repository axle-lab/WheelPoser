...
class Full2Pose_WheelPoser_Global_Finetune(pl.LightningModule):
    r"""
    3 Stage Pose S3
    Inputs - N IMUs + full positions, Outputs - pose
    """
    def __init__(self, config, pretrained_model):
        super().__init__()
        n_input = 12 * len(config.joints_set)
        n_output = joint_set.n_upper_body*6 if config.r6d == True else joint_set.n_upper_body*9
