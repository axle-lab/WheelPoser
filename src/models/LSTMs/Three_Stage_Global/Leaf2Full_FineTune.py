...
class Leaf2Full_WheelPoser_FineTune(pl.LightningModule):
    r"""
    3 Stage Pose S2
    Inputs - N IMUs + Leaf positions, Outputs - Full Joint pos
    """
    def __init__(self, config, pretrained_model):
        super().__init__()
        n_input = 12 * len(config.joints_set) + 3*3
        self.batch_size = config.batch_size
        # self.dip_model = RNN(n_input=n_input, n_output=15 * 3, n_hidden=64, bidirectional=True)
