...
class WHEELPOSER(Dataset):
    ...
    def load_data(self, normalize=False):
        ...
        # if self.train == "train":
        #     if self.config.mixed_imu_set == True:
        #         data_folder = self.config.processed_wheelposer_mixed_nn_ready_4
        #         data_files = [x.name for x in self.config.processed_wheelposer_mixed_nn_ready_4.iterdir() if ("wheelposer_am_smooth_final_test.pt" != x.name)]
        #     elif self.config.leave_one_out == True:
        #         data_folder = self.config.processed_wheelposer_leave_one_out_wu
        #         data_files = [x.name for x in self.config.processed_wheelposer_leave_one_out_wu.iterdir() if ("13" not in x.name)]
        #     elif self.config.video_data == True:
        #         data_folder = self.config.processed_wheelposer_video
        #         data_files = [x.name for x in self.config.processed_wheelposer_video.iterdir() if ("wheelposer_wu_smooth_final_test.pt" != x.name)]
        #     else:
        #         data_files = ["wheelposer_am_smooth_final_train.pt"]
        # else:
        #     # data_files = ["wheelposer_wu_smooth_final_test.pt"]
        #     data_folder = self.config.processed_wheelposer_category
        #     # data_files = ['wheelposer_wu_category_final_13_arm.pt']
        #     data_files = ['wheelposer_wu_category_final_13_arm.pt']
